...
int getOperatorPrecedence(const std::string& op){
    ...
    else if (op == "==" || op == "!=" || op == "<" || op == ">" || op == "<=" || op == ">=") return 0;
    return -1;
}
...
private:
    ...
    MemoryPtr<BinaryOperationNode> parseBinary(int prevPrecedence = 0);
    MemoryPtr<UnaryOperationNode> parseUnary(const string op, MemoryPtr<ASTNode> operand);
