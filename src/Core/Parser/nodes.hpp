...
/* ASTNodes checklist!!!
    ...
    * wait where the fuck are module and program
*/
...
public:
    ...
    AssignmentNode(VariableNode* variable, const string& op, MemoryPtr<ASTNode>& varValue): variable(variable), variableValue(std::move(varValue)) {
        this->value = op;
...
public:
    ...
    optional<string> defaultValue; // the default value of the parameter (optional)
...
public:
    string moduleName;
    array<MemoryPtr<ASTNode>> body;

    ModuleNode(const string& name) : moduleName(name) {
